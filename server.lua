...
AddEventHandler('qb-methcar:start', function()
	...
	if ItemAcetone ~= nil and ItemLithium ~= nil and ItemMethlab ~= nil then
		...
		TriggerClientEvent('QBCore:Notify', source, "Meth yapmak için yeterli malzemen yok!", 'error')
		end
	else
	TriggerClientEvent('qb-methcar:stop', _source)
	TriggerClientEvent('QBCore:Notify', source, " Meth için ana malzemelere sahip değilsin!", 'error')
	end
end)
...
AddEventHandler('qb-methcar:make', function(posx,posy,posz)
	...
	TriggerClientEvent('QBCore:Notify', source, "Meth Labaratuarın yok!", 'error')
	end
end)
